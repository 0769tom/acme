...
describe("POST /peer/transactions", function () {
    ...
    describe("Registering a delegate", function () {
                    ...
                    node.api.put("/transactions")
                        ...
                        .end(function (err, res) {
                            node.onNewBlock(function (err) {
                                ...
                                var transaction = node.acme.delegate.createDelegate(account.password, crypto.randomBytes(64).toString("hex"));
        ...
        it("When account has no funds. Should fail", function (done) {
            var transaction = node.acme.delegate.createDelegate(node.randomPassword(), node.randomDelegateName().toLowerCase());
            transaction.fee = node.Fees.delegateRegistrationFee;
        ...
        it("When account has funds. Username is uppercase, Lowercase username already registered. Should fail", function (done) {
            var transaction = node.acme.delegate.createDelegate(account2.password, account.username.toUpperCase());
        ...
        it("When account has funds. Username is lowercase. Should be ok", function (done) {
            ...
            var transaction = node.acme.delegate.createDelegate(account.password, account.username);
                    ...
                    node.api.put("/transactions")
                        ...
                        .end(function (err, res) {
                            ...
                            node.onNewBlock(function (err) {
                                ...
                                var transaction = node.acme.delegate.createDelegate(account2.password, account2.username);
                                ...
                                node.peer.post("/transactions")
                                    ...
                                    .end(function (err, res) {
                                        ...
                                        account2.username = node.randomDelegateName().toLowerCase();
                                        var transaction2 = node.acme.delegate.createDelegate(account2.password, account2.username);
